        ...
        <ul className="mt-2 max-h-48 space-y-0.5 overflow-y-auto">
          {reports.map((r) => {
            ...
            >
              ...
              <Link
                href={`/?report=${r.id}`}
                aria-label={`Open report ${r.name} in chat`}
                title="Open this report in chat"
                className="shrink-0 rounded-md p-1 text-zinc-400 opacity-0 transition-opacity hover:bg-black/5 hover:text-zinc-900 focus:opacity-100 group-hover:opacity-100 dark:hover:bg-white/10 dark:hover:text-zinc-100"
              >
                <svg
                  width="14"
                  height="14"
